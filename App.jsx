    ...
    <div style={{ minHeight:"100vh", background:"#fff5f7", padding:"20px", fontFamily:"'Arial',sans-serif" }}>
      ...
      <div style={{ display:"flex", gap:"10px", marginBottom:"20px" }}>
        {["home","tasks","guests","budget"].map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{
            ...
            background: tab===t?"#ff8fa3":"white", color: tab===t?"white":"black", fontWeight:"bold"
          }}>
      ...
      {tab==="home" && (
        <div>
          ...
          {/* D-day */}
          <div style={{ background:"#fff", padding:"20px", borderRadius:"20px", textAlign:"center", marginBottom:"20px", boxShadow:"0 5px 15px rgba(0,0,0,0.05)" }}>
            <h2 style={{ fontSize:"32px", color:"#ff6f91" }}>D-{dday ?? "?"}</h2>
            <input type="date" value={weddingDate} onChange={(e)=>setWeddingDate(e.target.value)}
              style={{ marginTop:"10px", padding:"10px", borderRadius:"12px", border:"1px solid #ddd", width:"60%" }} />
          </div>

          {/* 달력 */}
          <div style={{ background:"#fff", padding:"20px", borderRadius:"20px", boxShadow:"0 5px 15px rgba(0,0,0,0.05)", marginBottom:"60px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"10px" }}>
              <button onClick={()=>setMonthOffset(monthOffset-1)} style={{ padding:"5px 10px", borderRadius:"8px", background:"#ffe8f0", border:"none" }}>◀ 이전달</button>
              <h3>{year}년 {month+1}월</h3>
              <button onClick={()=>setMonthOffset(monthOffset+1)} style={{ padding:"5px 10px", borderRadius:"8px", background:"#ffe8f0", border:"none" }}>다음달 ▶</button>
            </div>
              ...
              {[...Array(daysInMonth)].map((_,i)=>{
                ...
                return (
                  <div key={i} onClick={()=>setSelectedDate(date)}
                    style={{ padding:"12px", borderRadius:"10px", background:hasEvent?"#ffccd5":"#f9f9f9", textAlign:"center", cursor:"pointer" }}>
                    {i+1}
                  </div>
                )
              })}
            ...
            {selectedDate && (
              <div style={{ marginTop:"10px" }}>
                ...
                <input placeholder="일정" value={eventText} onChange={(e)=>setEventText(e.target.value)}
                  style={{ padding:"5px", borderRadius:"10px", border:"1px solid #ddd", marginRight:"5px" }} />
                <button onClick={()=>{ setEvents([...events,{text:eventText,date:selectedDate}]); setEventText(""); }} style={{ padding:"5px 10px", borderRadius:"10px", background:"#ff8fa3", color:"#fff", border:"none" }}>추가</button>
              </div>
            )}
          </div>
          ...
          <button onClick={()=>{ navigator.clipboard.writeText(coupleId); alert("초대 코드 복사됨: "+coupleId); }}
            style={{ position:"fixed", bottom:"20px", right:"20px", background:"#ff8fa3", color:"#fff", border:"none", padding:"16px", borderRadius:"50%", fontSize:"20px", cursor:"pointer" }}>
            +
          </button>
        </div>
      )}

      {/* 할일/하객/예산 탭은 기존 통합본의 인스타 감성 UI 그대로 반영 */}
    </div>
  );
}